...
int main ( int argc, const char *argv[] )
{
    ...
    char           _output[512] = { 0 };    
    const char    *p_database_file = (void *) 0;
    key_value_db  *p_key_value_db = (void *) 0;

    // Parse command line arguments
    parse_arguments(argc, argv, &p_database_file);

    // Construct the database
    key_value_db_construct(&p_key_value_db, p_database_file);
